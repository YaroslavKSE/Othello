...
public class Board
{
    ...
    public void MarkCell(int x, int y, CellState playerColor)
    {
        ...
        if (Cells[x, y] != CellState.Empty) throw new InvalidOperationException("Cell is already occupied.");

        if (playerColor == CellState.Empty) throw new ArgumentException("Invalid player color.");

        if (!IsValidMove(x, y, playerColor)) throw new InvalidOperationException("This move is invalid.");
    ...
    public bool IsValidMove(int x, int y, CellState playerColor)
    {
        ...
        var opponentColor = playerColor == CellState.Black ? CellState.White : CellState.Black;
    ...
    public void FlipPieces(int x, int y, CellState playerColor)
    {
        ...
        foreach (var (dx, dy) in _directions)
        {
            ...
            while (currentX >= 0 && currentX < Size && currentY >= 0 && currentY < Size)
            {
                ...
                {
                    foreach (var (flipX, flipY) in piecesToFlip) Cells[flipX, flipY] = playerColor; // Flip the piece
    ...
    {
        var flippedPieces = new List<(int, int)>();
        var opponentColor = playerColor == CellState.Black ? CellState.White : CellState.Black;
